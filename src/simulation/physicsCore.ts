...
export function createArenaColliders(rapier: RapierRuntime, world: RapierWorld): void {
  const floorDesc = rapier.ColliderDesc.cylinder(0.06, ARENA_RADIUS)
  ...
  const rimRadius = ARENA_RADIUS - 0.04;
...
export function applyArenaContainment(body: RapierRigidBody, proxyGeometry: ProxyGeometry): void {
  ...
  const maxCenterDistance = Math.max(0.4, ARENA_RADIUS - Math.max(proxyGeometry.radiusWorld * 0.45, 0.4));
...
export function getStopCandidateReason(spinRpm: number, tiltDegrees: number, radialDistance: number): SimulationStopReason {
  if (radialDistance > ARENA_RADIUS + 0.35) {
    return 'arena_exit';
  }
...
export function getRingOutCandidateReason(radialDistance: number): SimulationStopReason {
  return radialDistance > ARENA_RADIUS - 0.15 ? 'arena_exit' : null;
}
...
function createTopColliders(
  ...
  const bodyDescCollider = rapier.ColliderDesc.cylinder(bodyHeight / 2, proxyGeometry.radiusWorld)
  ...
  if (options.includeBattleRing) {
    const ringDesc = rapier.ColliderDesc.cylinder(Math.max(bodyHeight * 0.16, 0.035), proxyGeometry.radiusWorld * 1.08)
...
function getClampedLaunchPosition(position: LaunchSettings['position'], radiusWorld: number): { x: number; z: number } {
  const maxDistance = Math.max(0.2, ARENA_RADIUS - radiusWorld - 0.35);
